...
\begin{frame}
    ...
    \begin{tikzpicture}[
            auto,
            inner sep=2mm,
            box/.style={rectangle, align=center, rounded corners=5pt, text width=\textwidth},
        ]
        \node[box] (unsl) {\Large Universidad Nacional de San Luis};
        \node[box, below=0mm of unsl] (dep) {\large Departamento de informática};
        \node[box, below=1mm of dep, fill=structure.fg, text=white] (title) {\setlength{\baselineskip}{1.5em}\LARGE Parametrizando un Sistema Informático para Obra Social Universitaria};
        \node[box, below=1mm of title] (author) {Estudiante: Iván Brocas};
        \node[box, below=-1mm of author] (dir) {Director: Mg. Alejandro Sánchez};
        \node[box, below=-1mm of dir] (co-dir) {Co-Director: Dr. Carlos Humberto Salgado};
        \end{tikzpicture}
\end{frame}
\begin{comment}
...
Además, nos centramos en el backend del sistema, que utiliza, entre otras, las siguientes tecnologías (slides).
\end{comment}
...
\end{frame}
\begin{comment}
Del objetivo general, se extraen los objetivos específicos
...
Los motores considerados fueton: Rulebook, Easy Rules, Jess, Drools y OpenL Tablets.
\end{comment}
...
\end{frame}
\begin{comment}
De las opciones exploradas hay algunas que fueron descartadas, dado que presentan características contradictorias con los objetivos de este proyecto:
...
\end{frame}
\begin{comment}
Esto nos deja con los principales candidatos siendo Drools y OpenL Tablets. Estas dos opciones resultan comparables en los aspectos considerados.

La principal diferencia fue la presencia de errores en la herramienta de gestión de las reglas para el caso de Drools, el editor DMN. Algunas de las funcionalidades no se comportaban de forma correcta. Los casos más notables era para la importación de definiciones de tipos desde clases java y la definición de tipos dentro de las tablas de decisión.

A raíz de esto, se optó por OpenL tablets.
\end{comment}
...
\end{frame}
\begin{comment}
Para ver el impacto de la integración de OpenL Tablets sobre la adaptabilidad del sistema, veamos lo que conlleva realizar un cambio en las reglas de cálculo.
En particular, se considera el siguiente cambio.

En el sistema este cambio implica el cambio o adición de más de 30 líneas de código, además de que requiere volver a compilar el sistema y desplegar la nueva versión para hacer los cambios efectivos.
\end{comment}
...
\end{frame}
\begin{comment}
En la slide se puede ver la tabla original que se debe cambiar, la cual especifica la regla correspondiente con el caso general para los voluntarios adherente mencionado anteriormente.
\end{comment}
...
\end{frame}
\begin{comment}
Por otro lado, utilizando el motor de reglas, lograr el mismo cambio requiere únicamente la adición de la última fila en la segunda tabla. Una vez guardado el archivo, el sistema detecta los cambios y vuelve a generar las clases wrapper, haciendo los cambios efectivos.
\end{comment}
...
\end{frame}
\begin{comment}
En base a los ejemplos se puede ver que
- Se consiguió la separación de las reglas de negocios del código fuente
- Realizar cambios en las reglas no requiere volver a compilar ni desplegar el sistema

Por otra parte, comparando las distintas versiones utilizando como métrica la cantidad de líneas de código obtenemos los resultados de la tabla. Para reducir el sesgo introducido por los formateadores en el caso del código fuente, se utilizaron dos formateadores: uno que sigue la guía de estilo de google y el formateador del language server utilizado por el IDE Eclipse.
\end{comment}
...
\end{frame}
\begin{comment}
Revisitando los objetivos planteados al inicio de la presentación y teniendo en cuenta los resultados presentados podemos ver que el esfuerzo necesario para afrontar cambios ya no implica los ítem especificados.

Las reglas fueron brevemente explicadas en la revisión literaria, así como consideraciones adicionales extraídas del código fuente correspondiente.

Con base en la reducción de líneas de código, se supone una mejora en la legibilidad de las reglas de negocio y, en particular, una mejora de la legibilidad del código fuente
\end{comment}
...
\end{frame}
\begin{comment}
Si bien se espera que la reducción en las líneas de código junto con la abstracción de algunos aspectos del funcionamiento interno del sistema resulté en una mejor comprensión por parte del personal de la obra social, esto queda por ser realmente puesto a prueba.

A pesar de que las reglas fueron separadas del código fuente del sistema, quedó por configurar OpenL Studio, sin el cual la gestión resulta un proceso no trivial. OpenL Studio cuenta con herramientas para facilitar la gestión de las reglas, dentro de las cuales se encuentra la validación de las tablas integrada con el editor, herramienta de la cual carecen otros editores de hojas de cálculo.
\end{comment}
...
\end{frame}
\begin{comment}
En base a las limitaciones, posibles continuaciones de este trabajo podrían ser la inclusión de OpenL Studio para facilitar la gestión de las reglas, así como la comprobación de la facilidad del personal de la obra social para comprender las reglas. Por ejemplo mediante un experimento o una estimación más involucrada de dicho esfuerzo.

Adicionalemnte, la continuación más natural, es la parametrización de otros aspectos del sistema, como por ejemplo el cálculo de las coberturas de las prácticas
\end{comment}
...
\end{frame}
\begin{comment}
Le agradezco a los profesores por su acompañamiento durante el desarrollo de este trabajo

Un agradecimiento especial al jurado, por su compromiso frente al aviso con poca antelación

Y a los presentes por su tiempo y atención
\end{comment}
